...
# 添加详细的输出信息
print("模型输出格式：")
print(f"检测结果数量: {len(results.xyxy[0])}")
print("\n检测结果详情：")
for det in results.xyxy[0]:
    x1, y1, x2, y2, conf, cls = det
    print(results.names)
    class_name = results.names[int(cls)]
    print(f"类别: {class_name} (ID: {int(cls)}), 置信度: {conf:.2f}")
    print(f"边界框坐标: [{x1:.1f}, {y1:.1f}, {x2:.1f}, {y2:.1f}]")
    print("---")
...
video_file = '/Users/shiki/Movies/Splatoon/match_9.mp4'
