...
# デバイス選択部分を修正
USE_COREML = True  # macOSでCore MLを使用する場合はTrueに設定
if USE_COREML:
    device = "coreml"
else:
...
def load_model(model_path, device):
    if device == "coreml":
        return ct.models.MLModel(model_path.replace('.pt', '.mlmodel'))
    else:
        return torch.hub.load('.', 'custom', path=model_path, source='local')

# # モデルロード部分を変更
# model = load_model('../models/the_model.pt', device)
# ocr_model = load_model('../models/ocr_model.pt', device)
# message_ocr_model = load_model('../models/message_ocr_model.pt', device)
...
# 推論部分を修正
def inference_frame(model, frame, device):
    if device == "coreml":
        # Core ML用の前処理
        input_image = cv2.resize(frame, (640, 640))
        input_dict = {'input_image': input_image}
        results = model.predict(input_dict)
        # Core MLの出力を標準化された形式に変換
        # Note: 実際の出力形式に合わせて調整が必要
        return standardize_results(results)
    else:
        # PyTorch用の推論
        return model(frame, 640)

# メインループ内の推論部分を変更
results = inference_frame(model, frame, device)
    ...
    while True:
        ...
        if count % frame_interval == 0:
            ...
            for det in results.xyxy[0].cpu().numpy():
                ...
                label = f"{results_names[int(cls)]}: {conf:.2f}"
                cv2.rectangle(preview_frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                cv2.putText(preview_frame, label, (int(x1), int(y1)-10), 
            ...
            preview_frame_resized = cv2.resize(preview_frame, None, 
                                             fx=preview_scale, 
                                             fy=preview_scale)
            cv2.imshow("YOLOv5 Detection", preview_frame_resized)
            ...
            # 処理時間のログ出力
            print(f"\nFrame {frame_count} timing:")
            print(f"Frame read: {frame_read_time*1000:.1f}ms")
            print(f"Color convert: {convert_time*1000:.1f}ms")
            print(f"YOLO detect: {detect_time*1000:.1f}ms")
            print(f"Preview: {preview_time*1000:.1f}ms")
